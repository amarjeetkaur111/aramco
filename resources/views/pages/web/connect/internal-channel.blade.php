...
<link href="{{asset('assets/web/css/aramco.css')}}" rel="stylesheet" />
    ...
    <div class="row justify-content-between align-items-center mt-5 mb-2">
      <div class="col-md-6 col-sm-6 col-4 d-flex justify-content-start align-items-center">
      ...
      <div class="col-md-6 col-sm-6 col-8 d-flex justify-content-end align-items-center">
        <a href="{{route('connect-newpost',['id'=> $id])}}" class="pagesTopNavLink"><span class="sicon blue-bg"><img src="{{asset('assets/web/images/icons/24/edit-white.svg')}}" class="img-fluid" /></span>Create new post</a>
      </div>
    </div>
      <div class="row">
        <div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 col-12">
          <div class="card border-0 rounded-3 mb-3 p-4 bg-aramco-grey" >
            <div class="card-header bg-transparent p-0 border-0 d-flex justify-content-start align-items-center">
              All Channels 
              <a class="btn p-0 btn-link ms-auto postListLinksCollapse d-lg-none d-block" data-bs-toggle="collapse" href="#postListLinks" role="button" aria-expanded="false" aria-controls="postListLinks"><i class="fa-solid fa-chevron-down"></i></a>
            </div>
            <div class="list-group mt-3  w-100 postListLinks" id="postListLinks">
              @foreach($list as $arr)
              <a href="{{route('connect-internalChannels',['id'=> $arr->id])}}" class="list-group-item list-group-item-action {{$arr->id == $id ? 'active' : ''}}" aria-current="true">
                  <i class="postListeDot">
                      <span class="line"><span class="inner"></span></span>
                    </svg>
                  </i>
                  <div class="connect-list">
                    <div class="connect-logo"><img src="{{asset('assets/web/images/connect/ChatGPT.svg')}}" class="img-fluid" /></div>
                    <div class="w-100">
                      <div class="connect-logo-title text-wrap text-break">{{$arr->title}}</div>
                    </div>
                  </div>
              </a>
              @endforeach
        ...
        <div class="col-xl-9 col-lg-9 col-md-12 col-sm-12 col-12">
          <div class="border-0 rounded-3 p-4 bg-aramco-grey post-List-card-container" > 
            @if(count($channel->channels_posts) < 1)
            <p>No Post Yet!</p>
            @else
            @foreach($channel->channels_posts as $k => $post)
            @php 
              ...
              success:function(data){
                ...
                if(data.success)
                {
                  if(likeStatus === 'false')
                  {
                    ...
                    $('#'+post_id).attr('data-status',false);
                    $('#'+post_id).find('.post-stat-icon').find('.post-like-icon').toggleClass('liked');
                    $("#liked_"+post_id).text(parseInt($("#liked_"+post_id).text())-1);
                  }
                }
